...
public ArrayList query(String time) {
	String path;
	String[] result;
	...
	try {
		...
		resultSeq=expression.executeQuery("for $Movie in doc('"+path+"')/ShowList/Movie where $Movie/Schedule/@data='"+time+"'"+" return concat(concat(concat(concat(concat(concat(concat(concat(concat(string($Movie/Name),' '),concat(string($Movie/Poster),' ')),concat(string($Movie/Director),' ')),concat(string($Movie/Actor),' ')),concat(string($Movie/Type),' ')),concat(string($Movie/Price),' ')),concat(string($Movie/Schedule/@data),' ')),concat(string($Movie/Schedule/@Item1),' ')),concat(string($Movie/Schedule/Item2),' '))");
		//resultSeq=expression.executeQuery("for $Movie in doc('D://software_document//myeclipse_files//Ticketing//WebRoot//ShowList.xml')/ShowList/Movie where $Movie/Schedule/@data='"+time+"'"+" return concat(concat(concat(concat(concat(concat(concat(concat(concat(string($Movie/Name),' '),concat(string($Movie/Poster),' ')),concat(string($Movie/Director),' ')),concat(string($Movie/Actor),' ')),concat(string($Movie/Type),' ')),concat(string($Movie/Price),' ')),concat(string($Movie/Schedule/@data),' ')),concat(string($Movie/Schedule/@Item1),' ')),concat(string($Movie/Schedule/Item2),' '))");
